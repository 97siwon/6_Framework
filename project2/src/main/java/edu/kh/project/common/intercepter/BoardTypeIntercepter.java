...
// 인터셉터
// 클라이언트 <-> 필터 <-> DispatcherServlet <-> 인터셉터 <-> 컨트롤러

// Dispatcher Servlet <-> 컨트롤러 사이에
// 요청 / 응답을 가로채서 특정 코드를 수행하는 기능(Spring 에서만 사용 가능)
// ** Dispatcher Servlet 이후에 동작하므로
//    각 종 Spring 어노테이션을 인식하고 bean을 DI 할 수 있다.
...
public class BoardTypeIntercepter implements HandlerInterceptor {
	...
	// 모든 뷰서 최종 결과를 생성하는 일을 포함한 모든 작업이 완료된 후 수행
	// (forward를 통해 응답화면이 생성 완료된 후 수행)
	// (보통 요청 처리 중에 사용한 자원을 반환하는데(close()) 많이 사용) 
	
	@Override
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
			throws Exception {
		
		// application scope에 게시판 이름 목록(boardTypeList)이 있는지 
		// 확인하여 없을 경우 DB에서 조회하여 세팅
